import {
  ...
  useEffect(() => {
    ...
    const countAllReplies = (replies) => {
      let count = replies.length; // Start with the number of top-level replies
    
      replies.forEach((reply) => {
        if (reply.replies && reply.replies.length > 0) {
          count += countAllReplies(reply.replies); // Add nested replies recursively
        }
      });
    ...
    const handleReply = useCallback( async (e) => {
      ...
      try {
        ...
        setPosts((prevPosts) =>
          prevPosts.map((p) =>
            p._id === post._id
              ? { ...p, replies: [...p.replies, data] } // add new reply to main replies
              : p
          )
        );
  ...
  const handleNestedReplyChange = (e,replyId, value) => {
    e.preventDefault()
  ...
  const handleAddNestedReply = async (e,replyId) => {
    e.preventDefault()
  ...
  {showComments&&(
   ...
   {post?.replies?.map((reply,index)=>(
      <Box key={index} mb={2} borderWidth="1px" borderRadius="md" p={2} borderColor={'gray.600'} fontSize={'sm'}>
            <Text fontSize="sm" color="gray.300" fontFamily={'sans-serif '}>{reply.username}</Text>	
            ...
            <FormControl>
                ...
                size={'sm'}
                  ...
                  onChange={(e) => 
                    ...
                    handleNestedReplyChange(e,reply._id, e.target.value)}//const handleNestedReplyChange = (replyId, value) => {
                    // setNestedReplies((prev) => ({
                    //   ...prev,
                    //   [replyId]: value,
                    // }));
                />
                 <Button
                  mt={2}
                  onClick={(e) => handleAddNestedReply(e,reply._id)}
                >
                  Reply
                </Button>
              </FormControl>
        ...
        <Flex gap={2} alignItems={"center"}>
          ...
          {countAllReplies(post.replies)} comments
          </Text>
        </Flex>
